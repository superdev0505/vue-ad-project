    ...
    <section :class="$style.wrapper">
        ...
        <div :class="$style.contentBody">
            <div :class="$style.left">
                <grid-list-block v-if="category.top_stories" :items="category.posts.data" title="Latest Deals" />
                <sponsored-deals-list-block v-if="category.sponsored" :items="category.sponsored.posts.data" title="Sponsored Deals" cat-url="/category/best-cell-phone-deals/homepage-featured" cat-url-label="See All Deals" sponsored/>
                <gift-guide-list-block v-if="category.giftGuides" :items="category.giftGuides.posts.data" title="Gift Guides" cat-url="/category/best-cell-phone-deals/gift-guide" cat-url-label="See All Gift Guides" />
                <repeater-list-block v-if="category.posts" :items="category.posts.data" title="All Deals" />
                <nuxt-link v-if="category.posts.data.length === 58" :to="'/' + category.permalink + '/page/2'" class="btn btn--primary btn--block">View More Deals</nuxt-link>
            </div>
...
<script>
    ...
    export default {
        ...
        async asyncData({ params, app, route, error }) {
            ...
            let path = 'best-cell-phone-deals'
            if (page > 1) {
                path += '/page/' + page
            }
            ...
            response.sponsored = response.children.data.find(item => item.slug === 'homepage-featured')
            response.giftGuides = response.children.data.find(item => item.slug === 'gift-guide')
